...
vec3 find_nearest(in vec3 ro, in vec3 rd, in float t_entry, out float t_end) {
  ...
  vec3 voxels_per_unit = float(grid_size) / (grid_max - grid_min);
  ...
  vec3 pos = clamp(floor(entry_pos), vec3(0), vec3(float(grid_size)));
  ...
  for (int steps = 0; steps < MAX_STEPS; ++steps) {
      ...
      if (texel.r+texel.b+texel.g > 0.0) {
          if (steps == 0) { t_end = t_entry; return texel.rgb; }
      ...
      if (tmax.x < tmax.y) {
          if (tmax.x < tmax.z) {
              pos.x += step.x;
              if (pos.x < 0 || pos.x >= float(grid_size)) break;
              axis = 0;
              tmax.x += delta.x;
          } else {
              pos.z += step.z;
              if (pos.z < 0 || pos.z >= float(grid_size)) break;
              axis = 2;
              tmax.z += delta.z;
          }
      } else {
          if (tmax.y < tmax.z) {
              pos.y += step.y;
              if (pos.y < 0 || pos.y >= float(grid_size)) break;
              axis = 1;
              tmax.y += delta.y;
          } else {
              pos.z += step.z;
