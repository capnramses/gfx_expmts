...
vec4 add_vec4_vec4( vec4 a, vec4 b ) { return ( vec4 ){ .x = a.x + b.x, .y = a.y + b.y, .z = a.z + b.z, .w = a.w + b.w }; }
...
vec4 mul_vec4_f( vec4 v, float f ) { return ( vec4 ){ .x = v.x * f, .y = v.y * f, .z = v.z * f, .w = v.w * f }; }
