// TODO - try more sub-pixel samples and average them

#include "apg_pixfont.h"
#define STBIR_DEFAULT_FILTER_UPSAMPLE STBIR_FILTER_BOX
#define STBIR_DEFAULT_FILTER_DOWNSAMPLE STBIR_FILTER_BOX
#define STB_IMAGE_RESIZE_IMPLEMENTATION
#include "../common/include/stb/stb_image_resize.h"
...
int main() {
  ...
  if ( !_start_gl() ) { return 1; }
  {
    ...
    int scale     = 1;
    output_pixels = (unsigned char*)malloc( w * scale * h * scale * n_channels );
    stbir_resize_uint8( text_img_mem, w, h, 0, output_pixels, w * scale, h * scale, 0, n_channels );

    stbi_write_png( "out.png", w * scale, h * scale, n_channels, output_pixels, w * scale * n_channels );

    texture = load_texture_from_mem( output_pixels, w * scale, h * scale, n_channels );
    free( text_img_mem );
  }
  ...
  const char* vertex_shader =
    ...
    "  v_st = a_vp * 0.5 + 0.5;"
    "  v_st.t = 1.0 - v_st.t;"
    "  gl_Position = vec4( a_vp.xy * 0.04 , 0.0, 1.0 );"
    "  gl_Position.x += sin( u_time * 0.15 );"
    "}";
