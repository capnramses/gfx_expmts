/*
By Anton Gerdelan, 2026.

Implementation of this shader
...
int find_nearest( in vec3 entry_pos, in vec3 rd, in float t_entry, in ivec3 n_cells, out float t_end, out vec3 vox_n ) {
  ...
  for ( int steps = 0; steps < MAX_STEPS; ++steps ) {
    ...
    if (USE_BRANCHLESS_DDA) {
      ...
      mask = lessThanEqual(t.xyz, min(t.yzx, t.zxy));
			
			//All components of mask are false except for the corresponding largest component
			//of sideDist, which is the axis along which the ray should be incremented.			
			
			cell_ijk += ivec3(vec3(mask)) * step;
      ...
      axis = int(dot( vec3(0,1,2), vec3(mask) ));
      vox_n = vec3(mask ) * -step; // The *step is to get -1 on the reverse sides.
		} else {
