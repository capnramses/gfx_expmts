...
mesh_t create_mesh_from_mem( const float* points_buffer, int n_points_comps, const uint32_t* pal_idx_buffer, int n_pal_idx_comps, const float* picking_buffer,
  ...
  if ( texcoords_buffer && n_texcoord_comps > 0 ) {
    glGenBuffers( 1, &texcoords_buffer_gl );
    glBindBuffer( GL_ARRAY_BUFFER, texcoords_buffer_gl );
    glBufferData( GL_ARRAY_BUFFER, sizeof( float ) * n_texcoord_comps * n_vertices, texcoords_buffer, GL_STATIC_DRAW );
  ...
  if ( normals_buffer && n_normal_comps > 0 ) {
...
void draw_mesh( shader_t shader, mat4 P, mat4 V, mat4 M, uint32_t vao, size_t n_vertices, texture_t* textures, int n_textures ) {
  for ( int i = 0; i < n_textures; i++ ) {
    GLenum tex_type = textures[0].is_array ? GL_TEXTURE_2D_ARRAY : GL_TEXTURE_2D;
  ...
  glUseProgram( 0 );
  for ( int i = 0; i < n_textures; i++ ) {
    GLenum tex_type = textures[i].is_array ? GL_TEXTURE_2D_ARRAY : GL_TEXTURE_2D;
    glActiveTexture( GL_TEXTURE0 + i );
...
void draw_textured_quad( texture_t texture, vec2 scale, vec2 pos ) {
  GLenum tex_type = texture.is_array ? GL_TEXTURE_2D_ARRAY : GL_TEXTURE_2D;
  glEnable( GL_BLEND );
