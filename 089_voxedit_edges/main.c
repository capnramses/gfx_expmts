...
int main() {
  ...
  shader_t voxel_shader;
  {
    const char vert_shader_str[] = {
      ...
      "  v_n.w = a_vn.w;\n"
      "  v_edge = a_vedge;\n"
      "  gl_Position = u_P * u_V * u_M * vec4( pos * 0.1, 1.0 );\n"
      "}\n"
    };
    ...
    const char frag_shader_str[] = {
      ...
      "  float edge_max = max( v_edge.x, max( v_edge.y, max( v_edge.z, v_edge.w ) ) );\n"
  ...
  while ( !should_window_close() ) {
    ...
    if ( text_timer > 0.1 ) {
      ...
      text_texture.w = w;
      text_texture.h = h;
      text_texture.n_channels = fps_n_channels;
      update_texture( &text_texture, fps_img_mem, false );
