/* Notes from Tim
- semaphore and fence stuff is optional if you dont care about performance
- you can leave them all as VK_NULL_HANDLE
- and just use vkWaitDeviceIdle()
- which simplifies Hello Triangle immensely.

- b/c semaphores etc are a big departure from earlier APIs, you could
- just fill up command queue, run it, waitdeviceidle, repeat -> simpler loop.
*/
...
bool create_vertex_buffer( size_t sz ) {
  VkBufferCreateInfo vb_info = (VkBufferCreateInfo){
    ...
    .size        = sz,                                   //
    .usage       = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,    // Can be >1 flag.
    .sharingMode = VK_SHARING_MODE_EXCLUSIVE             // Only used by gfx queue.
  };
  // Create, but no memory assigned yet.
  if ( VK_SUCCESS != vkCreateBuffer( gfx.device, &vb_info, NULL, &vertex_buffer ) ) {
    fprintf( stderr, "ERROR: Failed to create vertex buffer!\n" );
    return false;
  }
  VkMemoryRequirements mem_reqs;
  vkGetBufferMemoryRequirements( gfx.device, vertex_buffer, &mem_reqs );
  ...
  if ( VK_SUCCESS != vkAllocateMemory( gfx.device, &alloc_info, NULL, &vertex_buffer_memory ) ) {
    fprintf( stderr, "ERROR: Failed to allocated vertex buffer memory!\n" );
    return false;
  }

  vkBindBufferMemory( gfx.device, vertex_buffer, vertex_buffer_memory, 0 );

  void* data;
  vkMapMemory( gfx.device, vertex_buffer_memory, 0, vb_info.size, 0, &data );
  memcpy( data, vertices, (size_t)vb_info.size );
  vkUnmapMemory( gfx.device, vertex_buffer_memory );
