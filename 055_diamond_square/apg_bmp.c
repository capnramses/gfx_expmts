...
static bool _validate_bmp_file_header( struct bmp_file_header_t* file_header_ptr, struct entire_file_t record ) {
  if ( 'B' != file_header_ptr->file_type[0] || 'M' != file_header_ptr->file_type[1] ) {
    fprintf( stderr, "ERROR: not a BMP file\n" );
    return false;
  }
  // demoted to warning because some old images have invalid headers but still load. fuzzers will die on this though.
  if ( file_header_ptr->file_sz > record.sz ) {
    fprintf( stderr, "WARNING: BMP file is smaller %u than size given in file header %u\n", (uint32_t)record.sz, (uint32_t)file_header_ptr->file_sz );
  }
  if ( file_header_ptr->image_data_offset + APG_BMP_MIN_DATA_WIDTH > record.sz ) {
    fprintf( stderr, "ERROR: BMP file is too small for a valid file\n" );
    return false;
  }
  return true;
}
...
static bool _validate_bmp_dib_header( struct bmp_dib_BITMAPINFOHEADER_t* dib_header_ptr, struct entire_file_t record ) {
  ...
  if ( 0 == dib_header_ptr->w || 0 == dib_header_ptr->h ) {
    fprintf( stderr, "ERROR: BMP header gives 0-length dimensions\n" );
    return false;
  }
  if ( ( 32 == dib_header_ptr->bpp || 16 == dib_header_ptr->bpp ) &&
    ...
    fprintf( stderr, "ERROR: BMP header has invalid compression/bpp match\n" );
    return false;
  }
  if ( BI_RGB != dib_header_ptr->compression_method && BI_BITFIELDS != dib_header_ptr->compression_method && BI_ALPHABITFIELDS != dib_header_ptr->compression_method ) {
    fprintf( stderr, "ERROR: unsupported compression mode %u\n", dib_header_ptr->compression_method );
  }
  // 8k * 16 seems like a reasonable 'hey this is probably an accident' size
  if ( 0 == dib_header_ptr->w || 0 == dib_header_ptr->h || abs( dib_header_ptr->w ) > 7182 * 10 || abs( dib_header_ptr->h ) > 7182 * 16 ) {
    fprintf( stderr, "ERROR: width and height %ix%i in BMP header did not look like valid sizes\n", dib_header_ptr->w, dib_header_ptr->h );
    return false;
  }
...
unsigned char* apg_read_bmp( const char* filename, int* w, int* h, int* n_chans, int vertical_flip ) {
  ...
  default: // this includes 2bpp and 16bpp
    fprintf( stderr, "ERROR: bpp %u is not currently supported\n", dib_header_ptr->bpp );
    free( record.data );
    return NULL;
  } // endswitch
  ...
  *n_chans = n_dst_chans;
  ...
  if ( file_header_ptr->image_data_offset + ( unpadded_row + row_padding ) * height > record.sz ) {
    fprintf( stderr, "ERROR: BMP file was not large enough to describe its pixel data\n" );
    free( record.data );
    return NULL;
  }
  ...
  if ( has_bitmasks ) {
    ...
    if ( bitshift_rgba[0] < 0 || bitshift_rgba[1] < 0 || bitshift_rgba[2] < 0 || bitshift_rgba[3] < 0 ) {
      fprintf( stderr, "ERROR: invalid bitmasks in BMP header\n" );
      free( record.data );
      return NULL;
    }
  }
  ...
  if ( !dst_pixels_ptr ) {
    fprintf( stderr, "ERROR: malloc allocated no memory\n" );
    free( record.data );
    return NULL;
  }
    ...
    for ( uint32_t c = 0; c < width; c++ ) {
      ...
      if ( 32 == dib_header_ptr->bpp ) {
        ...
        if ( palette_offset + index * 4 + 2 >= record.sz ) {
          fprintf( stderr, "ERROR: BMP file size has no space for its palette\n" );
          free( record.data );
...
int apg_write_bmp( const char* filename, const unsigned char* pixel_data, int w, int h, int n_chans ) {
  if ( !filename || !pixel_data || w <= 0 || h <= 0 || n_chans <= 0 ) {
