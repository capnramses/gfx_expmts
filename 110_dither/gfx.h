...
/** Copy a sub-image from a framebuffer into an array.
 * @param x            Horizontal coordinate. Zero at the left, framebuffer width at the right.
 * @param y            Vertical coordinate. Zero at the bottom, framebuffer height at the top.
 * @param w            Width of sub-image to copy.
 * @param h            Height of sub-image to copy.
 * @param n_channels   Number of channels to copy.
 * @param pbo_pair_ptr If NULL then read without a pixel buffer.
 * @param data_ptr     Pointer to copy sub-image data to. This must be an array of at least size w * h * n_channels bytes.
 * @note               Mouse coordinates will needs a vertical flip eg. `y = fb_height - mouse_y`.
 * @warning            If x+w or y+h exceed the framebuffer dimensions the driver will most likely crash the program.
 * @warning            This call will block CPU/GPU sync and drop performance.
 *                     Use a pixel buffer pair via pbo_pair_ptr to avoid stalls at cost of latency and synchronisation.
 */
void gfx_read_pixels_bgra( int x, int y, int w, int h, int n_channels, gfx_pbo_pair_t* pbo_pair_ptr, uint8_t* data_ptr );

void gfx_screenshot();

/** Take and resize a screenshot into some pre-allocated memory.
 *  @note The caller must think about preserving the aspect ratio.
 */
void gfx_screenshot_thumbnail_rgb( int w, int h, uint8_t* img_ptr );
...
extern gfx_shader_t gfx_scaled_vc_shader;        /** A default shader with vertex points and colours, but no texture. Points are scaled by 0.1. */
