...
static inline mat4 perspective_offcentre_viewport( int vp_w, int vp_h, int subvp_x, int subvp_y, int subvp_w, int subvp_h, mat4 P_orig ) {
  ...
  float subvp_y_ndc = ( subvp_y / (float)vp_h ) * 2.0f - 1.0f;
  float subvp_w_ndc = ( (float)(float)subvp_w / (float)vp_w ) * 2.0f;
  ...
  mat4 M  = { { 0 } };
  M.m[0]  = 2.0f / (float)subvp_w_ndc;
  M.m[5]  = 2.0f / (float)subvp_h_ndc;
  M.m[10] = 1.0f;
  M.m[12] = -2.0f * (float)subvp_x_ndc / (float)subvp_w_ndc - 1.0f;
  M.m[13] = -2.0f * (float)subvp_y_ndc / (float)subvp_h_ndc - 1.0f;
  M.m[15] = 1.0f;
