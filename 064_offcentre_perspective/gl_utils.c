...
bool rebuild_framebuffer( framebuffer_t* fb_meta ) {
  assert( fb_meta );
  assert( fb_meta->width > 0 && fb_meta->height > 0 );
  assert( fb_meta->handle_gl > 0 );
  assert( fb_meta->depth_texture.handle_gl > 0 );

  glBindFramebuffer( GL_FRAMEBUFFER, fb_meta->handle_gl );
  {
    glFramebufferTexture2D( GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, fb_meta->output_texture.handle_gl, 0 );
    glFramebufferTexture2D( GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, fb_meta->depth_texture.handle_gl, 0 );
...
bool create_framebuffer( framebuffer_t* fb_meta ) {
  ...
  glGenFramebuffers( 1, &fb_meta->handle_gl );
  fb_meta->output_texture = create_texture_from_mem( NULL, fb_meta->width, fb_meta->height, 4, false, false );
  fb_meta->depth_texture  = create_texture_from_mem( NULL, fb_meta->width, fb_meta->height, 1, false, true );
  if ( !rebuild_framebuffer( fb_meta ) ) { return false; }
  return true;
}
