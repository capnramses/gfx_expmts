...
#define SHADER_BINDING_VPICKING 2 /* special colours to help picking algorithm */
...
mesh_t create_mesh_from_mem( const float* points_buffer, int n_points_comps, const float* colours_buffer, int n_colours_comps, const float* picking_buffer,
  ...
  GLuint points_buffer_gl = 0, colour_buffer_gl = 0, picking_buffer_gl = 0;
  ...
  if ( picking_buffer && n_picking_comps > 0 ) {
    glGenBuffers( 1, &picking_buffer_gl );
    glBindBuffer( GL_ARRAY_BUFFER, picking_buffer_gl );
    glBufferData( GL_ARRAY_BUFFER, sizeof( float ) * n_picking_comps * n_vertices, picking_buffer, GL_STATIC_DRAW );
  ...
  mesh_t mesh =
    ( mesh_t ){ .vao = vertex_array_gl, .points_vbo = points_buffer_gl, .colours_vbo = colour_buffer_gl, .picking_vbo = picking_buffer_gl, .n_vertices = n_vertices };
  return mesh;
}
...
texture_t create_texture_from_mem( const uint8_t* img_buffer, int w, int h, int n_channels, bool srgb, bool is_depth ) {
  ...
  update_texture( &texture, img_buffer );
  return texture;
}
...
void draw_mesh( shader_t shader, camera_t cam, mat4 M, uint32_t vao, size_t n_vertices ) {
  ...
  glProgramUniformMatrix4fv( shader.program_gl, shader.u_P, 1, GL_FALSE, cam.P.m );
  glProgramUniformMatrix4fv( shader.program_gl, shader.u_V, 1, GL_FALSE, cam.V.m );
...
void rebuild_framebuffer( framebuffer_t* fb, int w, int h ) {
  ...
  glBindFramebuffer( GL_FRAMEBUFFER, fb->handle_gl );
  {
    ...
    update_texture( &fb->output_texture, NULL );
    ...
    update_texture( &fb->depth_texture, NULL );
