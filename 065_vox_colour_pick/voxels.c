...
static void _memcpy_colour( block_type_t block_type, float* dest ) {
  ...
  for ( int v = 0; v < 6; v++ ) { memcpy( dest + v * N_CUBE_FACE_VERT_COMPS, colour_ptr, sizeof( float ) * N_CUBE_FACE_VERT_COMPS ); }
}

static void _memcpy_picking( int x, int y, int z, int face_idx, float* dest, size_t curr_float, size_t max_floats ) {
  ...
  // per the 6 vertices in this face
  assert( curr_float + N_CUBE_FACE_VERT_COMPS * 6 < max_floats );
  for ( int v = 0; v < 6; v++ ) { memcpy( &dest[curr_float + v * N_CUBE_FACE_VERT_COMPS], buff, sizeof( float ) * N_CUBE_FACE_VERT_COMPS ); }
}

chunk_vertex_data_t chunk_gen_vertex_data( const chunk_t* chunk ) {
  chunk_vertex_data_t data = ( chunk_vertex_data_t ){ .n_vp_comps = 3, .n_vc_comps = 3, .n_picking_comps = 3 };
  ...
  data.picking_ptr = malloc( data.sz );
...

        {
          ...
          for ( int i = 0; i < 6; i++ ) {
            ...
            if ( !ret || neighbour_block_type == BLOCK_TYPE_AIR ) {
              ...
              _memcpy_picking( x, y, z, i, data.picking_ptr, total_floats + floats_added_this_block, N_FULL_CUBE_FLOATS * chunk->n_non_air_voxels );
  ...
  assert( data.colours_ptr );
  data.picking_ptr = realloc( data.picking_ptr, data.sz );
  assert( data.picking_ptr );
