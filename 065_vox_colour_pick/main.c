...
int main() {
  ...
  while ( !should_window_close() ) {
    ...
    // TODO add a stencil here with a wee quad on it
    { // render colour IDs around mouse cursor
      bind_framebuffer( &picking_fb );
      viewport( 0, 0, picking_fb.width, picking_fb.height );
      clear_colour_and_depth_buffers( 0, 0, 0, 1 ); // TODO and stencil
      // enable stenciling
      // stencil quad
      // disable stenciling
      draw_mesh( cam, identity_mat4(), chunk_mesh.vao, chunk_mesh.n_vertices, false, 1.0f );
      uint8_t data[4] = { 0, 0, 0, 0 };
      read_pixels( mouse_x, picking_fb.height - mouse_y, 1, 1, 4, data );
      bind_framebuffer( NULL );
      printf( "mouse RGBA {%x %x %x %x}\n", data[0], data[1], data[2], data[3] );
      // clear or ignore stencil
    }
  }
