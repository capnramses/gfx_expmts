...
#define VOXEL_N_VN_COMPS 4
#define CUBE_FACE_N_NORMAL_FLOATS ( N_CUBE_FACE_VERTS * VOXEL_N_VN_COMPS )
#define FULL_CUBE_N_NORMAL_FLOATS ( CUBE_FACE_N_NORMAL_FLOATS * 6 )
#define FULL_CUBE_N_NORMAL_BYTES ( FULL_CUBE_N_NORMAL_FLOATS * sizeof( float ) )
...
chunk_vertex_data_t _test_cube_from_faces() {
  ...
  const float* colours[6] = { red, green, blue, cyan, violet, yellow };
  for ( int face_idx = 0; face_idx < 6; face_idx++ ) {
    memcpy( &cube.positions_ptr[float_idx], faces[face_idx], N_CUBE_FACE_BYTES );
    for ( int i = 0; i < N_CUBE_FACE_VERTS; i++ ) { memcpy( &cube.colours_ptr[float_idx + i * 3], colours[face_idx], sizeof( float ) * 3 ); }
...
uint8_t noisei( int x, int z, int max_height ) {
  int val = (sin(x * x * 0.01) * 2.0 - 1.0) * 10 + (cos(z * 0.021) * 2 - 1) * 13; // TODO diamond square
  val %= max_height;
...
chunk_t chunk_generate( int chunk_x, int chunk_z ) {
  ...
  for ( int z = 0; z < CHUNK_Z; z++ ) {
    for ( int x = 0; x < CHUNK_X; x++ ) {
      uint8_t height = noisei( chunk_x * CHUNK_X + x, chunk_z * CHUNK_Z + z, CHUNK_Y - 1 );
...
static void _memcpy_face_colour( block_type_t block_type, float* dest ) {
  assert( dest );
  ...
  for ( int v = 0; v < N_CUBE_FACE_VERTS; v++ ) { memcpy( dest + v * N_CUBE_FACE_VERT_COMPS, colour_ptr, sizeof( float ) * N_CUBE_FACE_VERT_COMPS ); }
}
...
static void _memcpy_face_picking( int x, int y, int z, int face_idx, float* dest, size_t curr_float, size_t max_floats ) {
  ...
  // per the 6 vertices in this face
  assert( curr_float + N_CUBE_FACE_VERT_COMPS * N_CUBE_FACE_VERTS < max_floats );
  for ( int v = 0; v < N_CUBE_FACE_VERTS; v++ ) { memcpy( &dest[curr_float + v * N_CUBE_FACE_VERT_COMPS], buff, sizeof( float ) * N_CUBE_FACE_VERT_COMPS ); }
}
...
static void _memcpy_face_normals( const chunk_t* chunk, int x, int y, int z, int face_idx, float* dest, size_t curr_float, size_t max_floats ) {
  ...
  assert( curr_float + VOXEL_N_VN_COMPS * N_CUBE_FACE_VERTS < max_floats );
  for ( int v = 0; v < N_CUBE_FACE_VERTS; v++ ) { memcpy( &dest[curr_float + v * VOXEL_N_VN_COMPS], buff, sizeof( float ) * VOXEL_N_VN_COMPS ); }
}

chunk_vertex_data_t chunk_gen_vertex_data( const chunk_t* chunk ) {
  chunk_vertex_data_t data = ( chunk_vertex_data_t ){ .n_vp_comps = 3, .n_vc_comps = 3, .n_vpicking_comps = 3, .n_vn_comps = VOXEL_N_VN_COMPS };
  data.vp_buffer_sz        = N_FULL_CUBE_BYTES * chunk->n_non_air_voxels;
  data.vn_buffer_sz        = FULL_CUBE_N_NORMAL_BYTES * chunk->n_non_air_voxels;
  data.positions_ptr       = malloc( data.vp_buffer_sz );
  assert( data.positions_ptr );
  data.colours_ptr = malloc( data.vp_buffer_sz );
  assert( data.colours_ptr );
  data.picking_ptr = malloc( data.vp_buffer_sz );
  assert( data.picking_ptr );
  data.normals_ptr = malloc( data.vn_buffer_sz );
  assert( data.normals_ptr );

  uint32_t total_floats         = 0;
  uint32_t total_normals_floats = 0;
  ...
  for ( int y = 0; y < CHUNK_Y; y++ ) {
    for ( int z = 0; z < CHUNK_Z; z++ ) {
      for ( int x = 0; x < CHUNK_X; x++ ) {
        ...
        assert( total_floats < N_FULL_CUBE_FLOATS * chunk->n_non_air_voxels );

        {
          ...
          for ( int face_idx = 0; face_idx < 6; face_idx++ ) {
            ...
            if ( !ret || neighbour_block_type == BLOCK_TYPE_AIR ) {
              memcpy( &data.positions_ptr[total_floats + floats_added_this_block], faces[face_idx], N_CUBE_FACE_BYTES );
              _memcpy_face_colour( our_block_type, &data.colours_ptr[total_floats + floats_added_this_block] );
              _memcpy_face_picking( x, y, z, face_idx, data.picking_ptr, total_floats + floats_added_this_block, N_FULL_CUBE_FLOATS * chunk->n_non_air_voxels );
              _memcpy_face_normals( chunk, x, y, z, face_idx, data.normals_ptr, total_normals_floats, FULL_CUBE_N_NORMAL_FLOATS * chunk->n_non_air_voxels );
              floats_added_this_block += N_CUBE_FACE_FLOATS;
              total_normals_floats += CUBE_FACE_N_NORMAL_FLOATS;
            }
          }
        }
  ...
  data.vp_buffer_sz  = total_floats * sizeof( float );
  data.vn_buffer_sz  = total_normals_floats * sizeof( float );
  data.positions_ptr = realloc( data.positions_ptr, data.vp_buffer_sz );
  assert( data.positions_ptr );
  data.colours_ptr = realloc( data.colours_ptr, data.vp_buffer_sz );
  assert( data.colours_ptr );
  data.picking_ptr = realloc( data.picking_ptr, data.vp_buffer_sz );
  assert( data.picking_ptr );
...
void chunk_free_vertex_data( chunk_vertex_data_t* chunk_vertex_data ) {
  ...
  free( chunk_vertex_data->colours_ptr );
