...
gfx_shader_t gfx_scaled_vc_shader, gfx_quad_texture_shader, gfx_default_textured_shader, gfx_skybox_shader, gfx_skysphere_shader;
gfx_mesh_t gfx_ss_quad_mesh, gfx_skybox_cube_mesh, gfx_sphere_mesh;
...
/// default dims to fit on laptop screen in a window
static const int _init_window_width = 1024, _init_window_height = 768;
static gfx_stats_t _stats;
static bool _fullscreen;
static int _swap_interval;
static float ss_quad_pos[]    = { -1.0f, 1.0f, -1.0f, -1.0f, 1.0f, 1.0f, 1.0f, -1.0f };
static float _skybox_points[] = { -1.0f, 1.0f, -1.0f, -1.0f, -1.0f, -1.0f, 1.0f, -1.0f, -1.0f, 1.0f, -1.0f, -1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 1.0f, -1.0f, -1.0f,
...
static int _strnlen( const char* str, int maxlen ) {
  int i = 0;
  while ( i < maxlen && str[i] ) { i++; }
  return i;
}

static void _strncat( char* dst, const char* src, const int dest_max, const int src_max ) {
  assert( dst && src );

  int dst_len   = _strnlen( dst, dest_max );
...
static void _init_skybox_cube() {
  gfx_mesh_params_t params = ( gfx_mesh_params_t ){ .points_buffer = _skybox_points, .n_points_comps = 3, .n_vertices = 36 };
...
gfx_shader_t gfx_create_shader_program_from_strings( const char* vert_shader_str, const char* frag_shader_str ) {
  ...
  if ( !res_a || !res_b || !linked || shader.program_gl == 0 ) { return gfx_scaled_vc_shader; }
  shader.loaded = true;
  return shader;
}
...
gfx_shader_t gfx_create_shader_program_from_files( const char* vert_shader_filename, const char* frag_shader_filename ) {
  ...
  _strncat( shader.vs_filename, vert_shader_filename, GFX_SHADER_PATH_MAX, GFX_SHADER_PATH_MAX );
  _strncat( shader.fs_filename, frag_shader_filename, GFX_SHADER_PATH_MAX, GFX_SHADER_PATH_MAX );
...
void gfx_shaders_reload() {
  for ( int i = 0; i < _n_shaders; i++ ) {
    ...
    vs_filename[0] = fs_filename[0] = '\0';
    _strncat( vs_filename, _registered_shaders_ptrs[i]->vs_filename, GFX_SHADER_PATH_MAX, GFX_SHADER_PATH_MAX );
    _strncat( fs_filename, _registered_shaders_ptrs[i]->fs_filename, GFX_SHADER_PATH_MAX, GFX_SHADER_PATH_MAX );
...
void gfx_fullscreen( bool fullscreen ) {
  ...
  if ( fullscreen ) {
    ...
    int w = 0, h = 0;
    glfwGetWindowSize( gfx_window_ptr, &w, &h );
    w = w > mode->width ? mode->width : w;
    h = h > mode->height - 64 ? mode->height - 64 : h; // -32 to make room for the title bar on Windows ( may still overlap start menu bar )
    // The refresh rate is ignored when no monitor is specified.
    glfwSetWindowMonitor( gfx_window_ptr, NULL, 0, 0, w, h, GLFW_DONT_CARE );
...
bool gfx_start( const char* window_title, const char* window_icon_filename, bool fullscreen ) {
  {
    ...
    { // CRITICAL PIECE HERE FOR GL 3.0 //////////////////////////////////////////
      ...
      // Possible values are GLFW_NATIVE_CONTEXT_API, GLFW_EGL_CONTEXT_API and GLFW_OSMESA_CONTEXT_API. This is a hard constraint. If no client API is
      // requested, this hint is ignored.
      glfwWindowHint( GLFW_CONTEXT_CREATION_API, GLFW_NATIVE_CONTEXT_API );

      glfwWindowHint( GLFW_CONTEXT_VERSION_MAJOR, 3 );
      glfwWindowHint( GLFW_CONTEXT_VERSION_MINOR, 2 ); // 3.2 is the latest version of OpenGL ES
    ...
    int w = _init_window_width;
    int h = _init_window_height;
    if ( fullscreen ) {
    ...
    gfx_window_ptr = glfwCreateWindow( w, h, window_title, gfx_monitor_ptr, NULL );
    if ( !gfx_window_ptr ) {
  ...
  { // check extensions and size limits here
    ...
    { // glcapsviewer reports 47% support
#ifdef DEBUG
  ...
  { // gfx_quad_texture_shader
    ...
    if ( !gfx_quad_texture_shader.loaded ) {
      fprintf( stderr, "ERROR: loading default quad textured shader\n" );
      return false;
    }
  } // gfx_quad_texture_shader
  { // gfx_skybox_shader
    ...
    if ( !gfx_skybox_shader.loaded ) {
      fprintf( stderr, "ERROR: loading gfx_skybox_shader\n" );
      return false;
    }
  } // gfx_skybox_shader
  _init_ss_quad();
...
void gfx_stop() {
  gfx_delete_shader_program( &gfx_skysphere_shader );
  ...
  gfx_delete_mesh( &gfx_sphere_mesh );
  glfwTerminate();
}
...
void gfx_draw_mesh( gfx_primitive_type_t pt, const gfx_shader_t* shader_ptr, mat4 P, mat4 V, mat4 M, uint32_t vao, size_t n_vertices,
  ...
  */
  glUniformMatrix4fv( shader_ptr->u_P, 1, GL_FALSE, P.m );
  glUniformMatrix4fv( shader_ptr->u_V, 1, GL_FALSE, V.m );
  glUniformMatrix4fv( shader_ptr->u_M, 1, GL_FALSE, M.m );

  glBindVertexArray( vao );

  glDrawArrays( mode, 0, (GLsizei)n_vertices );
  _stats.draw_calls++;
  _stats.drawn_verts += (uint32_t)n_vertices;
...
void gfx_draw_mesh_instanced( const gfx_shader_t* shader_ptr, mat4 P, mat4 V, mat4 M, uint32_t vao, size_t n_vertices, const gfx_texture_t* textures_ptr,
  ...
  glDrawArraysInstanced( GL_TRIANGLES, 0, (GLsizei)n_vertices, n_instances );
  _stats.draw_calls++;
  _stats.drawn_verts += (uint32_t)n_vertices;
...
void gfx_draw_textured_quad( gfx_texture_t texture, vec2 scale, vec2 pos, vec2 texcoord_scale, vec4 tint_rgba ) {
  assert( gfx_quad_texture_shader.program_gl );
  ...
  _stats.draw_calls++;
  _stats.drawn_verts += (uint32_t)gfx_ss_quad_mesh.n_vertices;
...
void gfx_skysphere_draw( float time_fac, mat4 P, mat4 V, const gfx_texture_t* time_texture_ptr ) {
  V.m[12] = V.m[13] = V.m[14] = 0.0f; // remove translation
  glDepthMask( GL_FALSE );
  glFrontFace( GL_CW ); // draw from inside
  glUseProgram( gfx_skysphere_shader.program_gl );
  glUniformMatrix4fv( gfx_skysphere_shader.u_P, 1, GL_FALSE, P.m );
  glUniformMatrix4fv( gfx_skysphere_shader.u_V, 1, GL_FALSE, V.m );
  glUniform1f( gfx_skysphere_shader.u_daytime, time_fac );
  if ( time_texture_ptr ) {
    glActiveTexture( GL_TEXTURE0 );
    glBindTexture( GL_TEXTURE_2D, time_texture_ptr->handle_gl );
  }

  glBindVertexArray( gfx_sphere_mesh.vao );

  glDrawArrays( GL_TRIANGLES, 0, (GLsizei)gfx_sphere_mesh.n_vertices );
  _stats.draw_calls++;
  _stats.drawn_verts += (uint32_t)gfx_sphere_mesh.n_vertices;
...
void gfx_screenshot() {
  ...
  assert( img_ptr ); // TODO graceful log and exit
...
void gfx_screenshot_thumbnail_rgb( int dw, int dh, uint8_t* dst_img_ptr ) {
  if ( !dst_img_ptr || dw == 0 || dh == 0 ) { return; }
  int src_w = 0, src_h = 0;
  gfx_framebuffer_dims( &src_w, &src_h );
  if ( src_w == 0 || src_h == 0 ) { return; }
  uint8_t* src_img_ptr = malloc( src_w * src_h * 3 );
  assert( src_img_ptr ); // TODO graceful log and exit
  glPixelStorei( GL_PACK_ALIGNMENT, 1 );
  glReadBuffer( GL_COLOR_ATTACHMENT0 );
  glReadPixels( 0, 0, src_w, src_h, GL_RGB, GL_UNSIGNED_BYTE, src_img_ptr ); // note can be eg float
  glPixelStorei( GL_PACK_ALIGNMENT, 4 );                                     // return to default

  for ( int dy = 0; dy < dh; dy++ ) {
    for ( int dx = 0; dx < dw; dx++ ) {
      int dst_idx = ( dy * dw + dx ) * 3;
      float xp    = dx / (float)dw;
      float yp    = ( ( dw - 1 ) - dy ) / (float)dh; // flip on Y (bit of a hack)
      int src_x   = (int)( xp * src_w );
      int src_y   = (int)( yp * src_h );
      int src_idx = ( src_y * src_w + src_x ) * 3;
      memcpy( &dst_img_ptr[dst_idx], &src_img_ptr[src_idx], 3 );
    }
  }
  free( src_img_ptr );
}
